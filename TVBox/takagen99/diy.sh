...
sed -i "/versionName/s#[0-9a-zA-Z_\.\'\"-]\+\$#\'版本号: '$diy_TIME'\'#" TVBoxOSC/app/build.gradl
...
#sed -i '/private View seriesFlagFocus = null;/a\    private V7GridLayoutManager mGridViewLayoutMgr = null;' TVBoxOSC/app/src/main/java/com/github/tvbox/osc/ui/activity/DetailActivity.java
#sed -i 's/mGridView.setLayoutManager(new V7GridLayoutManager(this.mContext, isBaseOnWidth() ? 6 : 7));/mGridView.setHasFixedSize(false);\n        this.mGridViewLayoutMgr = new V7GridLayoutManager(this.mContext, isBaseOnWidth() ? 6 : 7);\n        mGridView.setLayoutManager(this.mGridViewLayoutMgr);\n/g' TVBoxOSC/app/src/main/java/com/github/tvbox/osc/ui/activity/DetailActivity.java
#sed -i '/seriesAdapter.setNewData(vodInfo.seriesMap.get(vodInfo.playFlag));/i\        Paint pFont = new Paint();\n        Rect rect = new Rect();\n        List<VodInfo.VodSeries> list = vodInfo.seriesMap.get(vodInfo.playFlag);\n        int w = 1;\n        for(int i =0; i < list.size(); ++i){\n            String name = list.get(i).name;\n            pFont.getTextBounds(name, 0, name.length(), rect);\n            if(w < rect.width()){\n                w = rect.width();\n            }\n        }\n        w += 32;\n        int screenWidth = getWindowManager().getDefaultDisplay().getWidth()\/3;\n        int offset = screenWidth\/w;\n        if(offset <=1) offset =1;\n        if(offset > 6) offset =6;\n        this.mGridViewLayoutMgr.setSpanCount(offset);\n' TVBoxOSC/app/src/main/java/com/github/tvbox/osc/ui/activity/DetailActivity.java
#sed -i 's/FrameLayout/LinearLayout/g' TVBoxOSC/app/src/main/res/layout/item_series.xml
#sed -i 's/width=\"wrap_content\"/width=\"match_parent\"/g' TVBoxOSC/app/src/main/res/layout/item_series.xml
#sed -i 's/@dimen\/vs_190/match_parent/g' TVBoxOSC/app/src/main/res/layout/item_series.xml
